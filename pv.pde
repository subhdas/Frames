...
//**** POINT CLASS
//************************************************************************
class pt { float x=0,y=0,z=0; // added z component 
  // CREATE
  pt () {}
  pt (float px, float py) {x = px; y = py;};
  pt (float px, float py,float pz) {x = px; y = py; z = pz;}; // added z component
  // MODIFY
  ...
  pt roundOff() {x=round(x); y=round(y); z = round(z); return this;}                                                                     // vertex for drawing polygons between beginShape() and endShape()
...
class vec { float x=0,y=0,z=0; 
 ...
 vec set (float px, float py, float pz) {x = px; y = py; z = pz; return this;}; 
  vec setTo(float px, float py,float pz) {x = px; y = py; z = pz; return this;}; 
  vec setTo(vec V) {x = V.x; y = V.y; z = V.z; return this;}; 
  vec zero() {x=0; y=0;z=0; return this;}
  vec scaleBy(float u, float v,float w) {x*=u; y*=v; z *= w; return this;};
  vec scaleBy(float f) {x*=f; y*=f; z*=f; return this;};
  vec mul(float f) {x*=f; y*=f; z*=f; return this;};
  vec reverse() {x=-x; y=-y; z=-z; return this;};
  vec divideBy(float f) {x/=f; y/=f; z/=f; return this;};
  vec normalize() {float n=sqrt(sq(x)+sq(y)+sq(z)); if (n>0.000001) {x/=n; y/=n;}; return this;};
  vec add(float u, float v,float w) {x += u; y += v; z+= w; return this;};
  ...
  // OUTPUT VEC
  vec clone() {return(new vec(x,y,z));}; 
...
//************************************************************************
// create 
pt P() {return P(0,0,0); };                                                                            // make point (0,0)
pt P(float x, float y,float z) {return new pt(x,y,z); };                                                       // make point (x,y)
pt P(pt P) {return P(P.x,P.y,P.z); };                                                                    // make copy of point A
pt Mouse() {return P(mouseX,mouseY,0);};                                                                 // returns point at current mouse location
pt Pmouse() {return P(pmouseX,pmouseY,0);};                                                              // returns point at previous mouse location
pt ScreenCenter() {return P(width/2,height/2,500);}                                                        //  point in center of  canvas
...
pt R(pt Q, float a, pt C) {float dx=Q.x-C.x, dy=Q.y-C.y, dz=Q.z-C.z, c=cos(a), s=sin(a); return P(C.x+c*dx-s*dy, C.y+s*dx+c*dy,50); };  // Q rotated by angle a around point P
pt P(pt P, vec V) {return P(P.x + V.x, P.y + V.y,0); }                                                 //  P+V (P transalted by vector V)
...
// average 
pt P(pt A, pt B) {return P((A.x+B.x)/2.0,(A.y+B.y)/2.0,0); };                                          // (A+B)/2 (average)
pt P(pt A, pt B, pt C) {return P((A.x+B.x+C.x)/3.0,(A.y+B.y+C.y)/3.0,0); };                            // (A+B+C)/3 (average)
pt P(pt A, pt B, pt C, pt D) {return P(P(A,B),P(C,D)); };                                            // (A+B+C+D)/4 (average)

// weighted average 
pt P(float a, pt A) {return P(a*A.x,a*A.y,0);}                                                      // aA  
pt P(float a, pt A, float b, pt B) {return P(a*A.x+b*B.x,a*A.y+b*B.y,0);}                              // aA+bB, (a+b=1) 
pt P(float a, pt A, float b, pt B, float c, pt C) {return P(a*A.x+b*B.x+c*C.x,a*A.y+b*B.y+c*C.y,0);}   // aA+bB+cC 
pt P(float a, pt A, float b, pt B, float c, pt C, float d, pt D){return P(a*A.x+b*B.x+c*C.x+d*D.x,a*A.y+b*B.y+c*C.y+d*D.y,0);} // aA+bB+cC+dD 

// LERP
pt L(pt A, pt B, float t) {return P(A.x+t*(B.x-A.x),A.y+t*(B.y-A.y),0);}
...
vec cross(vec U, vec V) {
  float xx = (U.y*V.z) - (V.y*U.z);
  float yy = (V.x*U.z) - (U.x*V.z);
  float zz = (U.x*V.y) - (V.x*U.y);
  vec vc = new vec(xx,yy,zz);
  vc = scaleByVec(0.005,vc);
return vc; }
...
vec normedV(vec V){  
  float mag = sqrt(sq(V.x) +sq(V.y) +sq(V.z));
  float x = V.x/mag;
  float y = V.y/mag;
  float z = V.z/mag;  
  return new vec(x,y,z);
}
vec V(pt P) {return new vec(P.x,P.y,P.z); };                                                              // make vector from origin to P
vec V(float x, float y,float z) {return new vec(x,y,z); };                                                      // make vector (x,y)
vec V(pt P, pt Q) {return new vec(Q.x-P.x,Q.y-P.y,0);};                                                 // PQ (make vector Q-P from P to Q
vec U(vec V) {float n = n(V); if (n==0) return new vec(0,0,0); else return new vec(V.x/n,V.y/n,0);};      // V/||V|| (Unit vector : normalized version of V)
vec U(pt P, pt Q) {return U(V(P,Q));};                                                                // PQ/||PQ| (Unit vector : from P towards Q)
vec MouseDrag() {return new vec(mouseX-pmouseX,mouseY-pmouseY,0);};                                      // vector representing recent mouse displacement
vec scaleByVec(float f,vec V) {V.x*=f; V.y*=f; V.z*=f; return V;}; // modified scaling

// weighted sum 
vec W(float s,vec V) {return V(s*V.x,s*V.y,s*V.z);}                                                      // sV
vec W(vec U, vec V) {return V(U.x+V.x,U.y+V.y,0);}  
vec W(vec U, vec V,vec W) {return V(U.x+V.x+W.x,U.y+V.y+W.y,U.z+V.z+W.z);} // U+V 
...
vec W(float u, vec U, float v, vec V,float w, vec W) {return W(S(u,U),S(v,V),S(w,W));} 
// transformed 
vec R(vec V) {return new vec(-V.y,V.x,V.z);};                                                             // V turned right 90 degrees (as seen on screen)
vec R(vec V, float a) {float c=cos(a), s=sin(a); return(V(V.x*c-V.y*s,V.x*s+V.y*c,0)); };                                     // V rotated by a radians
vec S(float s,vec V) {return new vec(s*V.x,s*V.y,s*V.z);};                                                  // sV
vec Reflection(vec V, vec N) { return W(V,-2.*dot(V,N),N);};                                          // reflection

// Interpolation 
vec L(vec U, vec V, float s) {return new vec(U.x+s*(V.x-U.x),U.y+s*(V.y-U.y),0);};                      // (1-s)U+sV (Linear interpolation between vectors)
...
// measure 
float dot(vec U, vec V) {return U.x*V.x+U.y*V.y+U.z*V.z; }      // dot(U,V): U*V (dot product U*V)
...
//************************************************************************
// point / polygon
void show(pt P, float r) {ellipse(P.x, P.y, 2*r, 2*r);}; 
void showZ(pt P, float r) {translate(P.x, P.y, P.z);
sphere(r);translate(0,0,0);}; // draws circle of center r around P
...
void show(pt P, vec V) {line(P.x,P.y,P.z,P.x+V.x,P.y+V.y,P.z+V.z); }                                              // show V as line-segment from P 
...
void arrow(pt P, vec V, String S) {arrow(P,V); P(P(P,0.70,V),15,R(U(V))).label(S,V(-5,4,0));}       // show V as arrow from P and print string S on its side
void arrow(pt P, vec V) {
show(P,V);  
float n=n(V); if(n<0.01) return;
/*
float s=max(min(0.2,20./n),6./n);       // show V as arrow from P 
pt Q=P(P,V); vec U = S(-s,V); 
vec W = R(S(.3,U)); beginShape(); 
v(P(P(Q,U),W)); 
v(Q); 
v(P(P(Q,U),-1,W)); 
endShape(CLOSE);
