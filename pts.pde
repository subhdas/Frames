...
class pts {
 ...
 pt[] G = new pt [maxnv];                 // geometry table (vertices)
  pts() {}
  pts declare() {for (int i=0; i<maxnv; i++) G[i]=P(); return this;}               // init points (0,0,0)
  ...
  pts resetOnCircle(int k) { // init the points to be on a circle
    pt C = ScreenCenter(); 
    for (int i=0; i<k; i++) {
      addPt(R(P(C,V(0,-width/3,0)),2.*PI*i/k,C));
    };
    roundOff();
    return this;
    } 
  ...
  pts makeGrid (int w) { // make a 2D grid of w x w vertices
   for (int i=0; i<w; i++) for (int j=0; j<w; j++)  
     addPt(P(.7*height*j/(w-1)+.1*height,.7*height*i/(w-1)+.1*height,0));
   return this;
   }    
  ...
  pts IDs() {
    for (int v=0; v<nv; v++) { 
      ...
      if(v<10) label(G[v],str(v));  else label(G[v],V(-5,0,0),str(v)); 
      }
    noFill();
    return this;
    }
