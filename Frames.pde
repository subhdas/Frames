...
void draw() {      // executed at each frame
  ...
  //println("P.G[1] z value is : " + P.G[1].z);
  ...
  penFill(cyan,3);   showArrows(F1); // show the second frame F1 (will be hidden by blue iteration)    
 ...
 ///*
  ...
  E=F(E0);
  for(int i=0; i<k; i++) {
    F=F.of(F1rF0,zAdd);     // F * F0^{-1} * F1
    zAdd += inc;
    if(showFrames) {penFill(blue,3); showArrows(F);}
    E=F.of(E0rF0,zAdd);              // F * F0^{-1} * E0
    if(showFrames)  {pen(magenta,3,magenta); showArrow(E);}
    Fk = F(F);
    if(i==k-1) {zAdd = temp; zAdd =0;}
  }
  
  //for similarity function___________________________________________
  //compute all the variables to get similarity motion working
  preCompute(); 
  ...
  //Ft = F(F0,t,Fk); penFill(black,3);    showArrows(Ft); // show intermediate frame of morph (F0,t,Fk); // have to edit this function to make the motion
  Ft = Fk.FSIM(F0,t); penFill(black,3);    showArrows(Ft);
  Et=Ft.of(E0rF0,temp); penFill(magenta,3);   showArrow(Et); 
  //*/
  if(animating) {t+=0.01; if(t>=1) {t=1; animating=false;}} 

  pen(black,1,white); P.draw(3);
popMatrix();// end of d drawing
  //+++++++++++++++++++++++++
...
void keyPressed() { // executed each time a key is pressed: sets the "keyPressed" and "key" state variables, 
  ...
  if(key=='x') inc += 5;  // add z value
  if(key=='y') inc -= 5;  // add z value
  
  change=true; // to make sure that we save a movie frame each time something changes
  }
  void mouseWheel(MouseEvent event) {dz += event.getAmount(); change=true; println(" DZ value is : " + dz);}
